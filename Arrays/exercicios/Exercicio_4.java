...
public class Exercicio_4 {
	public static void main(String args[]) {
		...
		for (i = 0; i < 2; i++) {
			for (j = 0; j < 2; j++) {
				System.out.println("Sobre a primeira matriz, insira o número da linha " + (i + 1) + " e da coluna "
						+ (j + 1) + ": ");
				matriceOne[i][j] = ler.nextFloat();
			}
		}
		...
		for (i = 0; i < 2; i++) {
			for (j = 0; j < 2; j++) {
				System.out.println(
						"Sobre a segunda matriz, insira número da linha " + (i + 1) + " e da coluna " + (j + 1) + ".");
				matriceTwo[i][j] = ler.nextFloat();
			}
		}
